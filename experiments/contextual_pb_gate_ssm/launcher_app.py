...
always reflects the current CLI flags (including the C. Factorization variant).
...
GROUPS = [
    ...
    ("Plots & storyboards", lambda d: d in {
        "use_storyboard", "use_storyboard_compact", "use_comparison_storyboard",
        "use_unified_comparison", "use_animations"}, False),
    ("Robot model", lambda d: d == "robot_model", True),
    ("C. Factorization", lambda d: d.startswith("ctx_") or d in {"contextual_comparison", "match_to_contextual"}, True),
...
def image_validation_error(path: Path) -> str | None:
    """Return a short diagnostic when an image is empty, partial, or unreadable."""
    try:
        if path.stat().st_size == 0:
            return "the file is empty, usually because generation or synchronization was interrupted"
        with Image.open(path) as image:
            image.verify()
    except FileNotFoundError:
        return "the file disappeared while the page was refreshing"
    except (UnidentifiedImageError, OSError, ValueError) as exc:
        detail = str(exc).strip()
        return f"Pillow could not verify the image{f': {detail}' if detail else ''}"
    return None


def render_image_or_warning(container, path: Path, **image_kwargs) -> bool:
    """Render one image without allowing a corrupt artifact to crash Streamlit."""
    error = image_validation_error(path)
    if error is not None:
        container.warning(f"Skipped unreadable image `{path.name}`: {error}.")
        return False
    try:
        container.image(str(path), caption=path.name, **image_kwargs)
    except Exception as exc:
        # A synchronized file can change after validation but before Streamlit
        # reads it. Keep the rest of the run browser usable in that race.
        container.warning(f"Could not display `{path.name}`: {exc}")
        return False
    return True
...
VARIANT_HELP = {
    ...
    "contextual_ssm": "C. Factorization architecture (context ports set below).",
}
...
WIDGET_LABELS = {
    "use_storyboard": "Single-episode storyboard",
    "use_storyboard_compact": "Compact single-episode storyboard",
...
def robot_model_is_nonlinear(value) -> bool:
    """Accept current checkbox values and older saved string defaults."""
    return value is True or str(value).strip().lower() == "nonlinear"
...
def render_widget(dest, bools, values, override=_NO_OVERRIDE):
    ...
    if dest in bools:
        ...
        return st.checkbox(label, value=val, help=e["help"], key=f"w_{dest}")
    e = values[dest]
    ...
    if choices:
        ...
        return st.selectbox(label, choices, index=idx, help=helptext, key=f"w_{dest}")
    if tname == "int":
        if default is None:
    ...
    with tab_custom:
        ...
        if not trained_runs:
            ...
            if custom_gifs:
                ...
                render_image_or_warning(st, custom_gifs[0])
            if custom_pngs:
                st.subheader("Trajectory")
                render_image_or_warning(st, custom_pngs[0], width="stretch")
    ...
    with tab_browse:
        ...
        if not runs:
            ...
            if top[0].button("🔁 Re-plot (plot_only)"):
                replot_argv = ["--no_show_plots", "--no_animations", "--plot_only", sel]
            ...
            if gifs:
                ...
                for g in gifs:
                    render_image_or_warning(st, g)
            if pngs:
                st.subheader("Figures")
                img_cols = st.columns(2)
                for i, p in enumerate(pngs):
                    render_image_or_warning(img_cols[i % 2], p, width="stretch")
            if pdfs:
                st.subheader("PDFs")
                for p in pdfs:
