...
\end{itemize}
The minimal context is designed so that the controller can learn the waiting strategy without
access to the freeze schedule \eqref{eq:freeze-step}: a rising \(\sigma_t\) near the wall
(\(\alpha_t\approx 1\)) signals that the gate has stabilised and it is safe to aim.
